...
class LLMGateway:
    ...
    def _build_client(self, config: LLMConfig) -> AsyncOpenAI:
        ...
        base_url_env = defaults.get("base_url_env", f"{config.provider.upper()}_BASE_URL")
        base_url = (
            config.base_url
    ...
    async def chat_stream(self, messages: list, **kwargs) -> AsyncIterator:
